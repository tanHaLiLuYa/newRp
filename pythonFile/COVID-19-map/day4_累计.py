...
pieces_list=[
        {"max": 999999, "min": 5000, "label": "5000+","color":"red"},
...
def map_visualmap(sequence, date) -> Map:
    ...
    c.set_global_opts(legend_opts=opts.LegendOpts(is_show=False),title_opts=opts.TitleOpts(title="CBIC：全国各省累计确诊_至4月8号",subtitle="数据来源于丁香园"),visualmap_opts=opts.VisualMapOpts(is_piecewise=True,\
        pieces=pieces_list))
    # c = (
    #     Map()
...
df3 = df2[["provinceName","province_confirmedCount","date"]]
# df3.set_index("date",inplace=True)
df3 = df3.pivot("date","provinceName","province_confirmedCount")
...
timeLine.render('./output/全国疫情累计确诊动态地图0929.html')



# provincesList = df3["provinceName"].drop_duplicates().values.tolist()
# print(df3.shape)
# print(df3["provinceName"])
# df3 = df3.set_index(df3["provinceName"].drop_duplicates().values.tolist())
# print(df3.head())
