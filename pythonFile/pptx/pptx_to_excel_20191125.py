...
file_name = "W46MS_Audit_Weekly_Report_20191122.pptx"
# 文件名不包含日期，字符串为25
do_file_name = file_name[:25]+file_name[len(file_name)-5:len(file_name)]
# print(do_file_name)

# 更新文件名称
back_file_name = do_file_name.replace(".pptx", "_backup{}.pptx".format(
    time.strftime("_%Y_%m_%d_%H_%M", time.localtime())))
new_file_name = do_file_name.replace(".pptx", "{}.pptx".format(
    time.strftime("_%Y_%m_%d_%H_%M", time.localtime())))
...
def get_id_slide_bytitle(prs_=prs, title_name="total_title"):
    ...
    for n in range(0, len(prs_.slides)):
        dic_[list_title[n]] = prs_.slides.index(
            prs_.slides.get(slide_id=list_id[n]))
    return dic_
# 设置字体格式函数1,
            ...
            for col in range(b_col, ppt_max_c):
                for row in range(b_row, ppt_max_r):
                    ...
                    if c_select.number_format == "General" or c_select.number_format == "@":
                        c.text = str(
                            c_select.value if c_select.value != None else "--")
                    elif c_select.number_format == "0.0":
                        c.text = str(round(c_select.value, 1) if c_select.value !=
                                     None and c_select.value != "--" else "--")
                        ...
                        c.text = str(c_select.value)[:10].replace(
                            "-", "/") if c_select.value != None and c_select.value != "--" else "--"
                    elif c_select.number_format == "h:mm":
                        c.text = str(c_select.value)[
                            :5] if c_select.value != None else "--"
                    else:
                        c.text = str(round(c_select.value) if c_select.value !=
                                     None and c_select.value != "--" else "--")
                    setfont(c, f_size)
            if rank_col:
                # 写入rank list
                ...
                for row in range(b_row+1, ppt_max_r):
                    ...
                    if isinlist(getmax(list_rank, nums=color_num, small=reverse_fill), c_select.value)and reverse_fill:
                        ...
                        fill_c(ishape=shape, irow=row, range_area_begin=0,
                               range_area_end=ppt_max_c-1,
                               r_color="白色")
    print("{}修改成功 in {}>>>>>>>>".format(ppt_shapename,
                                        (ppt_title+"--p"+str(get_id_slide_bytitle()[ppt_title]+1))))
...
def copy_text(sheetnames, ppt_title, ppt_shapename, para_index,
    ...
    for shape in page_name.shapes:
        if shape.name == ppt_shapename:
            ...
            # len ws range[0] 返回第一行的 列数,len ws rang 直接返回行数
            if ":" not in excel_cell:
                c_p.text = str(ws[excel_cell].value)
                setfont(shape, fsize=f_size[0],
                        alig=PP_ALIGN.LEFT, fbold=f_bold[0])
            else:
                c_p.text = ""
...
def chart_in_data(sheetnames, excel_range, ppt_title, ppt_shapename, num_format="0"):
    ws = wb[sheetnames]
    ws_range = ws[excel_range]
    # "p"+str(ppt_title+1)#没有0页的PPT需要加1，有的不需要
    page_name = prs.slides[get_id_slide_bytitle()[ppt_title]]
            ...
            for col in range(0, max_col):
                for row in range(0, max_row):
                    if ws_range[row][col].number_format == "General":
                        ...
                        list_ .append(
                            ws_range[row][col].value if ws_range[row][col].value else None)
                dic_[col] = list_
                list_ = []  # 清空历史数据
            # 写入chart中
            chart_data.categories = tuple(dic_[0][1:])
            for col in range(1, max_col):
                chart_data.add_series(name=dic_[col][0], values=tuple(
                    dic_[col][1:]), number_format=num_format)
            # shape.chart.chart_type()

            shape.chart.replace_data(chart_data)
    print("{}   修改成功     in    {} 页  >>>>>>>>".format(ppt_shapename,
                                                      (ppt_title+"--p"+str(get_id_slide_bytitle()[ppt_title]+1))))
...
class shape_():
    ...
    def __init__(self, ppt_title, excel_sheetname):
        self.ppt_title = ppt_title
        self.excel_sheetname = excel_sheetname
    # 修改表格方法

    def add_table(self, shapename, begin_index_list, cell_range, rank_col=False, color_num=False,
                  color_str_min="粉色", color_str_max="蓝色", f_size=9,
                  reverse_fill=None):
        ws = wb[self.excel_sheetname]
        ws_range = ws[cell_range]
        # "p"+str(ppt_slidesindex+1)#没有0页的PPT需要加1，有的不需要
        # 用ppttitle获取index 返回某一页
        page_name = prs.slides[get_id_slide_bytitle()[self.ppt_title]]
        ...
        for shape in page_name.shapes:
            ...
            if shape.name == shapename:
                ppt_max_r = get_maxrow_maxcol(shape.table)[0]
                ppt_max_c = get_maxrow_maxcol(shape.table)[1]
                b_col = begin_index_list[1]
                b_row = begin_index_list[0]
                ...
                for col in range(b_col, ppt_max_c):
                    for row in range(b_row, ppt_max_r):
                        ...
                        if c_select.number_format == "General" or c_select.number_format == "@":
                            ...
                            c.text = str(round(
                                c_select.value, 1) if c_select.value != None and c_select.value != "--" else "--")
                        elif c_select.number_format == "0%":
                            c.text = str(round(c_select.value*100) if c_select.value != None and c_select.value != "--" else " ") \
                                + ("%" if c_select.value !=
                                   None and c_select.value != "--" else "--")
                            ...
                            c.text = str(round(
                                c_select.value) if c_select.value != None and c_select.value != "--" else "--")
                        setfont(c, f_size)
                list_rank = []
                if rank_col:
                    # 写入rank list
                    ...
                    for row in range(b_row+1, ppt_max_r):
                        ...
                        if isinlist(getmax(list_rank, nums=color_num, small=reverse_fill), c_select.value)and reverse_fill:
                            fill_c(ishape=shape, irow=row, range_area_begin=0,
                                   range_area_end=ppt_max_c-1, r_color=color_str_min)
                        elif isinlist(getmax(list_rank, nums=color_num), c_select.value):
                            fill_c(ishape=shape, irow=row, range_area_begin=0,
                                   range_area_end=ppt_max_c-1, r_color=color_str_max)
                        else:
                            fill_c(ishape=shape, irow=row, range_area_begin=0,
                                   range_area_end=ppt_max_c-1, r_color="白色")
                print("{}   修改成功     in    {} 页  >>>>>>>>".format(
                    shapename, (self.ppt_title+"--p"+str(get_id_slide_bytitle()[self.ppt_title]+1))))
    # 修改图表方法

    def add_chart(self, shapename, excel_range, num_format="0"):
        ws = wb[self.excel_sheetname]
        ws_range = ws[excel_range]
        # "p"+str(ppt_title+1)#没有0页的PPT需要加1，有的不需要
        page_name = prs.slides[get_id_slide_bytitle()[self.ppt_title]]
    ...
    def add_text(self, shapename, excel_cell, para_index, f_size=[14], f_bold=[False]):
        ws = wb[self.excel_sheetname]
        # "p"+str(ppt_title+1)#没有0页的PPT需要加1，有的不需要
        page_name = prs.slides[get_id_slide_bytitle()[self.ppt_title]]
    ...
    def change_arrow(self, excel_range):
        page_name = prs.slides[get_id_slide_bytitle()[self.ppt_title]]
        ws = wb[self.excel_sheetname]
        ws_range = ws[excel_range]
        for shape in page_name.shapes:
...
a = shape_("4.人员问题—导购员和店员产品介绍对比", "NOTE10点检 EUP")

a.add_table(shapename="表格 2", begin_index_list=[3, 1], cell_range="E29:R40")
a.add_chart("总体结果-chart", "T28:V35")
a.add_text("title-text", "U45", 0)

# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# print("begin working ....................")
# copy_text(sheetnames="Appendix",excel_cell="B4:D4",self.ppt_title="Appendix.1 分公司别表现分析_整体得分",ppt_shapename="title-text",\
