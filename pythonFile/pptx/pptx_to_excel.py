...
def chart_in_data(sheetnames,excel_range,
    ...
    for shape in page_name.shapes:
        if shape.name == ppt_shapename:
            ...
            max_row=get_shapeofrange(shape)[0]
            max_col=get_shapeofrange(shape)[1]
            dic_={}
            list_=[]
            ####读取excel数据
            for col in range(0,max_col):
                for row in range(0,max_row):
                    list_ .append(str((ws_range[row][col]).value))
                dic_[col]=list_
                list_=[] #清空历史数据   
            #####写入chart中            
            chart_data.categories = tuple(dic_[0][1:])
            for col in range(1,max_col):
                chart_data.add_series(name=dic_[col][0],values=dic_[col][1:],number_format=num_format)
...
# copy_table(sheetnames="严重问题",excel_range="B22:H30",ppt_title="2.本周主要结果—问题门店复查改善情况",ppt_shapename="复查门店汇总-table",ppt_begin_index=[2,0])
