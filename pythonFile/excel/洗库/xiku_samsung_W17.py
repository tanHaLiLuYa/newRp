...
root_path = r"E:\work\tpp\samsung\2021年\04月\W17\洗库"


os.chdir(root_path)

fileName = "【W17】【洗库】0423_1500.xlsx"  
...
for rowNumber in range(2, ws2.max_row+1):
    ...
    for colNumber in range(column_index_from_string(starColumnName), column_index_from_string(endColumnName)+1):
        ...
        datiren = ws2["BR{}".format(rowNumber)].value if ws2["BR{}".format(rowNumber)].value else "none"
        datiren =datiren[0:1]
        # ————————————————————————————————————————————————————————————————————————————
        ...
        if testCol in ["Z", "AA","AC","AB"] and testValue[0:1] != "1" :  #, 
            fillYorN = True
        ...
        #4.1.2 A52多色出样
        if testCol =="AT" and otherValue(testCol,rowNumber, 5)=="Y" and otherValue(testCol,rowNumber, 1)>0 and testValue[0:1] not in ["4"] :
            if imageType =="3" and otherValue(testCol,rowNumber, 1)>3:
                fillYorN = True
            if imageType =="3" and otherValue(testCol,rowNumber, 1)<=3:
                fillYorN = True if  testValue[0:1] =="5" else False
            if imageType!= "3" :
                if otherValue(testCol,rowNumber, -1)[0:1] in ["2","4"]:
                    fillYorN = True
                else:
                    fillYorN = True if  testValue[0:1] in ["5","1"] else False

        #灯箱片
        if testCol=="AW" and otherValue(testCol,rowNumber, -1)[0:1] !="店":
            weizhi = int (otherValue(testCol,rowNumber, -1)[0:1] )
            weizhiStr =("Y"*weizhi +"N"*(3-weizhi)) if weizhi<3 else "Y"*3
            weiZhilist = list(weizhiStr)
            print(weiZhilist)
            fuGailist =[]
            for t in ["AR","AX","AY"]:
                testV = ws2["{}{}".format(t,rowNumber)].value
                fuGailist.append(testV)
            xianclist=[]
            xianclist.append("Y") if testValue.find("1.")!=-1 else xianclist.append("N")
            xianclist.append("Y") if testValue.find("2.")!=-1 or testValue.find("3.") !=-1 else xianclist.append("N")
            xianclist.append("Y") if testValue.find("4.")!=-1 else xianclist.append("N")
            for i in [0,1,2]:
                if weiZhilist[i]=="Y" and fuGailist[i]=="Y" and xianclist[i]=="N":
                    fillYorN = True 
            
                
        #4.2.2 4.2.2 Fold2灯箱片更换Foldable Combo KV

        if testCol in ["AZ"] and testValue[0:1] not in ["1","4", "3"] :
            fillYorN = True
        #4.2.3 A52 新品物料露出 BA 1

        #4.2.4 A52标识灯露出
        if testCol =="BB" and otherValue(testCol,rowNumber, -3)=="Y" and imageType=="3" and testValue[0:1] !="1" :
            fillYorN = True        
        if testCol =="BC" and otherValue(testCol,rowNumber, -4)=="Y" and imageType=="3" and testValue[0:1] not in["1","3"]:
            fillYorN = True   
        # LFD A52 
        if testCol =="BD" and testValue[0:1] not in ["1", "3"] :
            fillYorN = True

        #4.3.1 最新促销KV物料露出 BE 1 
        
        #A52陈列规范
        if  testCol in ["BF","BG"] and ws2["AY{}".format(rowNumber)].value =="Y":
            if testValue[0:1] !="1" and testValue.find("不考核")==-1 :
                fillYorN = True
            elif testValue[0:1] =="4" and testValue.find("不考核")!=-1 and imageType=="3":
                fillYorN = True
            elif testValue[0:1] =="5" and testValue.find("不考核")!=-1 and imageType in ["1","2"]:
                fillYorN = True
        #体验内容 规范
        if testCol in ["BH","BI","BJ","BK","BL","BM"] and testValue[0:1] !="1" and testValue.find("不考核")==-1:
            fillYorN =True


        #操作题目
        if testCol in ["BN","BO","BP","BQ"] and testValue[0:1] !="1" and testValue.find("无三星")==-1:
            fillYorN =True
        #人员
        if testCol == "CD" and testValue[0:1] not in ["1","2"] :
            fillYorN = True 
        if testCol == "CE" and testValue[0:1] not in ["1","5","6"] :
            fillYorN = True 
        ...
        #答题人姓名 正确与否
        if testCol == "BS" and otherValue(testCol,rowNumber,-1)[0:1] =="1":
            testp = otherValue(testCol,rowNumber,-46) if otherValue(testCol,rowNumber,-46)  else "无"
            YesOrNo ,name = namematch(testValue,testp)
            ws2["BS{}".format(rowNumber)].value = name
            if not YesOrNo:
                fillYorN = True 
        ...
        #     fillYorN = True
        if testCol == "CH" and testValue[0:2]not in ["7.", "8.", "9."]:
            fillYorN = True
