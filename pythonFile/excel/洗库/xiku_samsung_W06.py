...
root_path = r"E:\work\tpp\samsung\2021年\02月\W06\洗库"


os.chdir(root_path)

fileName = "【W06】【洗库】0202_1650.xlsx"  
...
for rowNumber in range(2, ws2.max_row+1):
    ...
    for colNumber in range(column_index_from_string(starColumnName), column_index_from_string(endColumnName)+1):
        ...
        datiren = ws2["CG{}".format(rowNumber)].value if ws2["CG{}".format(rowNumber)].value else "none"
        datiren =datiren[0:1]
        # ————————————————————————————————————————————————————————————————————————————
        ...
        if testCol in ["AB", "Z", "AA","AC","AX","AY","BB","BC","BD","BY","CQ"] and testValue[0:1] != "1" :  #, 
            fillYorN = True
        ...
        #4.2.1 新年促销KV A4立牌露出 BB 1

        # 4.3.2 Buds pro机模或真机露出BC 1
        #4.3.3 Fit2 机模或真机露出 BD 1

        # 4.4.1 新品台陈列
        if testCol == "BE" and imageType in ["1","2"]  and investorType in ["SES","SIS"]  :
            fillYorN = True if testValue[0:1] not in ["1", "5"]  else False

        #4.4.1 新品区陈列
        if testCol =="BF" and testValue[0:1] not in ["1","3"] and imageType in ["2"] :  
            fillYorN = True
        # 4.7.1 新品台陈列规范
        ...
        #4.4.2 S21系列 5G精品陈列
        if testCol =="BI" and testValue[0:1] not in ["1"] and otherValue(testCol,rowNumber,1)=="是":  
            fillYorN = True
        #4.4.3 S21 Ultra 5G黑色陈列套装组
        if testCol =="BK" and imageType in ["1","2"] and  testValue[0:1] !="1" and otherValue(testCol,rowNumber,2)=="Y":  
            fillYorN = True        
        if testCol =="BL" and imageType in ["3"] and  testValue[0:1] !="1" and otherValue(testCol,rowNumber,1)=="Y": 
            fillYorN = True  
        #4.4.4 S21+5G 紫色陈列套装组  4.4.5 S21 5G 陈列套装组
        if testCol in ["BN","BP"] and otherValue(testCol,rowNumber,1)=="Y" and  testValue[0:1] !="1":
            fillYorN = True  
        #4.6.1 S21系列5G手机保护壳露出
        if testCol =="BR" and testValue[0:1] =="3":
            fillYorN = True        
        #4.6.2 配件桌陈列
        if testCol =="BS" and testValue[0:1] =="2":
            fillYorN = True   
        #4.6.3 配件墙陈列
        if testCol =="BU" :
            if otherValue(testCol,rowNumber,-1)[0:1] in ["1","3"]:
                fillYorN = True   if testValue.find("2.") == "-1" or testValue.count(".")>1 else False
            elif otherValue(testCol,rowNumber,-1)[0:1]=="2":
                fillYorN = True   if testValue.find("2.") == "-1" or testValue.find("3.") == "-1" or testValue.count(".")!=2 else False
            elif otherValue(testCol,rowNumber,-1)[0:1]=="4":
                fillYorN = True   if testValue.find("2.") == "-1" or testValue.find("3.") == "-1" or testValue.find("1.") == "-1"or testValue.count(".")!=3 else False
            elif otherValue(testCol,rowNumber,-1)[0:1]=="5":
                fillYorN = True   if testValue.find("2.") == "-1"  or testValue.find("1.") == "-1"or testValue.count(".")!=2 else False
            else:
                fillYorN = True   if testValue.find("5.") == "-1"  or testValue.find("6.") == "-1"or testValue.count(".")!=1 else False
        if testCol =="BV" and testValue[0:1] not in ["1","5","6"] and  imageType in ["1","2"] :
            fillYorN = True  

        #    472 体验台桌下灯箱与产品保持同侧
        if testCol =="BW" and testValue[0:1] not in ["1","5","3"] and  imageType =="3" :
            fillYorN = True         
        # 4.7.3 机模陈列整齐
        if testCol =="BX" and testValue[0:1] not in ["1","3"]:
            fillYorN = True       
        #4.7.4 产品及陈列规范 BY 1
        if testCol =="BZ" and testValue[0:1] not in ["1","4"]:
            fillYorN = True     
        #4.5.1 所有手机真机 Retail Mode 程序100%自动运行
        if testCol =="CA" and testValue[0:1] not in ["1","4"]:
            fillYorN = True  
        #4.5.2 价签自动跳转（Flod2 不考核） 4.5.3 电子价签（“价格信息”app）是否更新了最新的促销活动
        if testCol in ["CB","CC"] and testValue[0:1] not in ["1","5"]:
            fillYorN = True          
        #人员
        if testCol == "CD" and testValue[0:1] not in ["1","2"] :
            fillYorN = True 
        if testCol == "CE" and testValue[0:1] not in ["1","5"] :
            fillYorN = True 
        if testCol == "CF" and investorType=="SES" and testValue[0:1] not in ["1","3"] :
            fillYorN = True 
        if testCol == "CH" and otherValue(testCol,rowNumber,-1)[0:1] =="1":
            testp = otherValue(testCol,rowNumber,-61) if otherValue(testCol,rowNumber,-61)  else "无"
            YesOrNo ,name = namematch(testValue,testp)
            ws2["CH{}".format(rowNumber)].value = name
            if not YesOrNo:
                fillYorN = True 
        ...
        # 违规  
        if testCol == "CS" and testValue[0:2]not in ["1.", "2.", "3."]:
            fillYorN = True
        if testCol == "CU" and testValue[0:2]not in ["7.", "8.", "9."]:
            fillYorN = True
        # 填充颜色 记录问题到B列
        # ————————————————————————————————————————————————————————————————————————————
        if fillYorN:
