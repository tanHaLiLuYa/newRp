...
root_path = r"E:\work\tpp\samsung\2021年\W04\洗库"


os.chdir(root_path)

fileName = "【W04】【洗库】0122_1430.xlsx"
...
for rowNumber in range(2, ws2.max_row+1):
    ...
    for colNumber in range(column_index_from_string(starColumnName), column_index_from_string(endColumnName)+1):
        ...
        if testCol in ["AB", "Z", "AA","AC","BB","BD","BV","CP"] and testValue[0:1] != "1" :  #, 
            fillYorN = True
        ...
        #4.3 LFD（电视屏幕）是否有播放S21系列5G 预售KV画面内容
        if testCol =="BC" and testValue[0:1] not in ["1","3"] :  
            fillYorN = True
        #4.4 春节促销KV露出(纸质物料) BD 1

        # Fold2 机模/真机
        if testCol == "BE" and otherValue(testCol,rowNumber,1)=="Y":
            fillYorN = True if testValue[0:1] != "1" else False

        #新平台
        if testCol == "BH" and imageType=="1."  and investorType in ["SES","SIS"]  :
            fillYorN = True if testValue[0:1] not in ["1", "4"]  else False
        if testCol == "BI" and imageType=="1." and otherValue(testCol,rowNumber,-3)=="Y" and investorType in ["SES","SIS"]  :
            fillYorN = True if testValue[0:1] not in ["1", "4"]  else False

        # W21 机模/真机
        if testCol == "BK" and otherValue(testCol,rowNumber,1)=="Y" and investorType in ["SIS","SES"]:
            fillYorN = True if testValue[0:1] != "1" else False
        #   W21 5G使用托架陈列
        if testCol== "BM" and investorType in ["SES","SIS"] and otherValue(testCol,rowNumber,-1)=="Y":
            fillYorN= True if testValue[0:1]!="1" else False

        #  W21 5G托架使用规范   
        if testCol== "BN"  and investorType in ["SES","SIS"] and otherValue(testCol,rowNumber,-2)=="Y":
            fillYorN= True if testValue[0:1]!="1" else False
        # S20 FE 多色
        if testCol== "BO" and imageType=="1." and otherValue(testCol,rowNumber,2)=="Y":
        ...
        if testCol== "BP" and imageType=="2." and otherValue(testCol,rowNumber,1)=="Y":
        ...
        if testCol =="BR"  and imageType=="2." and otherValue(testCol,rowNumber,-1)=="Y":
            fillYorN= True if testValue[0:1]!="1" else False
        #4.10 新形象门店S20 FE 3机托架陈列
        if testCol =="BS"  and imageType=="1." and otherValue(testCol,rowNumber,-2)=="Y":
            fillYorN= True if testValue[0:1] not in ["1", "7"]   else False   

        #4.14 体验台桌下灯箱与产品保持同侧
        if testCol =="BT" and imageType=="2." and investorType in ["SES","SIS"]:
        ...
        # 违规  
        if testCol == "CR" and testValue[0:2]not in ["1.", "2.", "3."]:
            fillYorN = True
        if testCol == "CT" and testValue[0:2]not in ["7.", "8.", "9."]:
            fillYorN = True
        # 填充颜色 记录问题到B列
        # ————————————————————————————————————————————————————————————————————————————
        if fillYorN:
