...
def merge_xlsx(path, filenames, sheet_name, output_filename):
      # 定义一个空list
    contentList = []
    dicHead = {}
    ...
    for i in range(len(filenames)):
        ...
        sheet_name_data = read_xlsx.sheet_by_name(sheet_name) # 查看指定sheet_name的数据
        title = sheet_name_data.row_values(0)  # 表头
        
        #找到关键列
        for  k in range(len(title)):
            column_x = k if title[k] == "A4.【必填】请记录三星导购员姓名和身份证号码最后四位数字" else 30
        for j in range(1, sheet_name_data.nrows):  # 逐行记录到 data
            data.append(sheet_name_data.row_values(j)[:column_x+1] )
        
        content = pd.DataFrame(data)  # 转换为DF
        content.columns = title[:column_x+1]
...
path = r'E:\work\tpp\samsung\2021年\汇总\4-7月已查导购员\file'
filenames = file_name(path)
merge_xlsx(path, filenames,'check1', "合并")  # 合并文件中第一个表的数据，输出到 output中
