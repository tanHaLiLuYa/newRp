...
root_path = r"C:\Users\16930\Desktop\照片"
middle_path = r"C:\Users\16930\Desktop\照片\新建文件夹"
aim_path = r"C:\Users\16930\Desktop\照片\aim"
file_name = r"最新导购员照片表.xlsx"
split_by = "CBI负责人"
#照片路径
pic_dir =r"G:\CBIC\导购员20190828"
...
for root ,dirs, files in excel_dirs:
    for f in files:
        ...
        wb_pic =openpyxl.load_workbook(excel_path)
        ws=wb_pic["Sheet1"]
        pic_id=1
        for row in range(2,ws.max_row+1):
            id=str(ws[f"S{row}"].value)+"__"+str(ws[f"T{row}"].value)+"."
            for i in list_p_id:
                if id in i :
                    print(id, os.path.join(dic_[i],i),pic_id)
                    image_address=os.path.join(dic_[i],i)
                    img=Image(image_address)
                    img.anchor = f"U{row}"
                    img.width  =70
                    img.height =80
                    ws.add_image(img,f"U{row}")
                    ws[f"U{row}"].value = pic_id
                    pic_id += 1
